...
export interface AIProvider {
  ...
  summarize?(responses: Array<{ aiName: string; content: string }>, apiKey: string): Promise<string>;
}
...
class MockAIProvider implements AIProvider {
  ...
  async search(query: string, _apiKey: string): Promise<string> {
  ...
  async summarize(responses: Array<{ aiName: string; content: string }>, _apiKey: string): Promise<string> {
    // モック実装：1秒待機してダミー要約を返す
    await new Promise((resolve) => setTimeout(resolve, 1000));
    const aiNames = responses.map(r => r.aiName).join('、');
    return `${aiNames}からの回答を要約しました。\n\nこれはモックの要約データです。実際の実装では、各AIの回答を分析して有用な要約を生成します。`;
  }
}
